...
/// [ReactiveStateInitializerCallback] is a class that extends `SingletonState` to manage global state reactively,
...
/// Example usage:
/// ```dart
/// final connectionState = ReactiveStateInitializerCallback<ConnectionElement>(
...
/// ```
class ReactiveStateInitializerCallback<T> extends SingletonState<T?> {
  static final Map<UKey, ReactiveStateInitializerCallback> _instances = {};
  ...
  ReactiveStateInitializerCallback._(this._initializer, this._onStateChange)
      : super(null) {
    _initialize();
  }

  /// Factory constructor to create and manage a single instance of `ReactiveStateInitializerCallback` per key.
  ///
  ...
  /// );
  /// ```
  factory ReactiveStateInitializerCallback(
      {required T Function() initializer,
      required T Function(T newState) onStateChange}) {
    UKey key = UKey();
    if (_instances[key] == null) {
      _instances[key] =
          ReactiveStateInitializerCallback<T>._(initializer, onStateChange);
    }
    return _instances[key] as ReactiveStateInitializerCallback<T>;
  }
  ...
  /// ```
  @override
  void resetState() => value = _initializer();
