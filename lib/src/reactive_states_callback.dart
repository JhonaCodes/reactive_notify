...
/// [ReactiveStateCallback] is a class that extends `SingletonState` to manage global state reactively,
...
/// Example usage:
/// ```dart
/// final connectionState = ReactiveStateCallback<ConnectionElement>(
...
/// ```
class ReactiveStateCallback<T> extends SingletonState<T> {
  static final Map<UKey, ReactiveStateCallback> _instances = {};
  ...
  ReactiveStateCallback.state(this._defaultValue, this._onStateChange)
      : super(_defaultValue);

  /// Factory constructor to create and manage a single instance of `ReactiveStateCallback` per type.
  ///
  ...
  /// ```
  ///
  factory ReactiveStateCallback(T initialValue,
      {T Function(T newState)? onStateChange}) {
    UKey key = UKey();
    if (_instances[key] == null) {
      _instances[key] =
          ReactiveStateCallback<T>.state(initialValue, onStateChange);
    }
    return _instances[key] as ReactiveStateCallback<T>;
  }
